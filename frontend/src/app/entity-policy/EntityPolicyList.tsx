...
const ROUTING_PATH = "/entityPolicyList";

const UMGMT_ENTITYPOLICY_LIST = gql`
  query umgmt_EntityPolicyList(
    $limit: Int
    $offset: Int
    $orderBy: inp_umgmt_EntityPolicyOrderBy
    $filter: [inp_umgmt_EntityPolicyFilterCondition]
  ) {
    umgmt_EntityPolicyCount(filter: $filter)
...
const EntityPolicyList = observer((props: EntityListProps<EntityPolicy>) => {
  ...
  const {
    ...
    goToParentScreen,
    entityListState
  } = useEntityList<EntityPolicy>({
    listQuery: UMGMT_ENTITYPOLICY_LIST,
  ...
  return (
    <DataTable
      ...
      columnDefinitions={["entityName", "action"]}
...
registerEntityList({
  component: EntityPolicyList,
  caption: "screen.EntityPolicyList",
  screenId: "EntityPolicyList",
  entityName: ENTITY_NAME,
  menuOptions: {
